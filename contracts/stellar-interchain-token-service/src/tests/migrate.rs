...
use crate::tests::utils::{setup_env, setup_upgrader};
...
fn migrate_succeeds() {
    ...
    let token_manager = client.mock_all_auths().token_manager_address(&token_id);
    let interchain_token = client.mock_all_auths().interchain_token_address(&token_id);
    ...
    env.as_contract(&client.address, || {
        let flow_key = legacy_storage::FlowKey {
            ...
            epoch: current_epoch, // NOTE: Might throw now
        };
    ...
    upgrader_client.mock_all_auths();
